...
```{r}
diamonds %>%
  glm(price ~ carat, data = ., family = 'gaussian') %>%
  tabglm()
```
...
Vamos a generar unos datos con algna variable binaria:

```{r}
mis_datos <- diamonds %>%
              mutate(good_cut = as.numeric(cut == 'Very Good' | cut == 'Premium'))
```

Un modelo probit se vería así:
